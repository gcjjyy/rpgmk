...
void redraw(const char *filename, int width, int height) {
    ...
    int x = 0;
    int y = 0;
    int image_count = calc_image_count(filename, width, height);
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        printf("Usage: %s <image> <palette> <width> <height>\n", argv[0]);
        return 0;
    }
    ...
    redraw(argv[1], atoi(argv[3]), atoi(argv[4]));
